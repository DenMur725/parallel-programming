#include <mpi.h> // «адача 3 є11  Ѕыстра€ сортировка с простым сли€нием
#include <ctime>
#include <iostream>
#include <fstream>
using namespace std;
...
int main(int argc, char * argv[]) {
	...
	int* displs = NULL;									// массив смещений в векторе дл€ начальной передачи данных
	int* sendcounts = NULL;								// массив длин подвекторов дл€ начальной передачи данных
	int* size_buf = NULL;								// массив длин подвекторов дл€ работы сортировки

	int n = 1, k = 0;	// n=2^k						// параметры бинарного делени€ массива
	double* part_mas = NULL;							// массив мортируемый каждым процессом
	double* tmp_mas = NULL;								// побочный массив дл€ сли€ни€ массивов
